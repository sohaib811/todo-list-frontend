...
const TaskViews = ({ _id, title, isCompleted, createdAt, getTasks }) => {
  ...
  const completeTask = async (e) => {
    ...
    e.stopPropagation();
    try {
      await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/tasks/${_id}`, {
  ...
  return (
    <div className="border rounded">
      {/* Accordion Trigger */}
      <button
        className={`w-full text-left p-4 bg-[#ddd4cc] border-t border-b border-l border-r border-gray-300 ${
          isOpen ? "bg-gray-200" : ""
        }`}
      ...
      {isOpen && (
        <div className="p-4 bg-[#ece9e4] w-full">
          <div className="flex">
            <h2 className="font-bold mr-1">Completed: </h2>{" "}
          ...
          <button
            class="bg-[#fecaca] text-red-500 py-2 px-4 rounded w-full mt-2"
            onClick={deleteTask}
          >
            Delete
          </button>
